// Loomabelle: pure-JS embroidery pipeline (+ HEIC support) → .DST + color preview + palette.txt
const $ = (s)=>document.querySelector(s);
const yearEl = $('#year'); if (yearEl) yearEl.textContent = new Date().getFullYear();
const statusEl = $('#status');
const setStatus = (m, cls='') => { if (!statusEl) return; statusEl.textContent = m; statusEl.className = `status ${cls}`; };
...
// ---------- HEIC support (loads only when needed) ----------
...
fileInput.addEventListener('change', async ()=>{
  ...
  try{
    ...
    // quick preview so you know it worked
    const maxSide = 1200;
    ...
    console.error(e);
    img = null; processBtn.disabled = true;
    setStatus('Could not read that image. Try a JPG/PNG.', 'error');
  }
});

// ---------- Main processing (pure JS hatch fill; simple outline) ----------
processBtn.addEventListener('click', async ()=>{
  if (!img) return;
  processBtn.disabled = true; downloadA.classList.add('disabled'); palA.classList.add('disabled');
  ...
  try{
    ...
    // 0) draw (cap size for mobile reliability)
    ...
    for (let ci=0; ci<masks.length; ci++){
      ...
      // simple outline: trace edge pixels and stitch around (coarse but effective)
      if (outline){
        const edgePts = marchingSquaresOutline(masks[ci], W, H);
        if (edgePts.length > 2){
          // scale & center to mm
          const mmPts = edgePts.map(([x,y])=>[(x-cx)*mmPerPx, (y-cy)*mmPerPx]);
          plan.stitches.push({ x:mmPts[0][0], y:mmPts[0][1], jump:true });
          for (let i=1;i<=mmPts.length;i++){
            lineStitch(plan.stitches, mmPts[i-1], mmPts[i % mmPts.length], 7);
          }
        }
      }
    }
    ...
    downloadA.href = dstUrl; downloadA.classList.remove('disabled');
    ...
    palA.href = paletteUrl; palA.classList.remove('disabled');
...
// ---------- Simple marching-squares outline (coarse) ----------
function marchingSquaresOutline(mask, W, H){
  // Find a starting edge pixel
  let sx=-1, sy=-1;
  for (let y=1;y<H-1 && sy<0;y++){
    for (let x=1;x<W-1;x++){
      if (mask[y*W+x] && !mask[y*W+(x-1)]) { sx=x; sy=y; break; } // left edge
    }
  }
  if (sx<0) return [];
  ...
  for (let step=0; step<maxSteps; step++){
    pts.push([x,y]);
    // marching squares: look at 2x2 block
    ...
    // simple edge-follow rules (favor clockwise)
    if (code===0 || code===5 || code===10) { // ambiguous/empty—try to move along
      // move forward
    } else if (code===1 || code===3 || code===9 || code===11){ // go right
      dir=0; x++;
    } else if (code===2 || code===6 || code===7 || code===14){ // go down
      dir=1; y++;
    } else if (code===4 || code===12 || code===13 || code===8){ // go left/up-ish
      dir=2; x--;
    } else { x++; } // fallback
...
function writeDST(plan){
  ...
  // Basic fields (machine software usually ignores most text, but ST/CO matter)
  put(`LA:LOOMABELLE\n`,0);
