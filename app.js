// Loomabelle: pure-JS pipeline + HEIC support → .DST
...
// ---------- HEIC support (loads on demand) ----------
async function heicToJpeg(file){
  if (!window.heic2any){
    await new Promise((res,rej)=>{
      const s=document.createElement('script');
      s.src='https://cdn.jsdelivr.net/npm/heic2any@0.0.4/dist/heic2any.min.js';
      s.onload=res; s.onerror=()=>rej(new Error('HEIC converter failed to load')); document.head.appendChild(s);
    });
  }
...
// ---------- Main processing (pure JS) ----------
processBtn.addEventListener('click', async ()=>{
  if (!img) return;
  processBtn.disabled = true; downloadA.classList.add('disabled');
  setStatus('Processing…');

  try{
    const colors = clamp(Number($('#colors').value)||4,2,5);
    const removeBg = $('#removeBg').checked;
    let outline = $('#outline').checked; // disabled in fallback below
    ...
    // 5) hatch fill directly from masks (outline skipped in pure JS mode)
    ...
    downloadA.href = url; downloadA.classList.remove('disabled');
