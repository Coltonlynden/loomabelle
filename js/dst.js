...
export function writeDST(stitches, hoopMM, opts={}){
  ...
  for(let si=0; si<stitches.length; si++){
    ...
    if(insertColorStops && s.color !== lastColor){
      bytes.push(0xF1, 0x00, 0x00); // color change
      lastColor = s.color;
    }

    const dxMM = s.x - prev.x, dyMM = s.y - prev.y;
    const step = 7; // mm per chunk
...
function colorBlocks(stitches){ let blocks=0, last=null; for(const s of stitches){ if(last===null || s.color!==last){ blocks++; last=s.color; } } return blocks||1; }
...
function calcExtents(stitches){
  ...
  for(const s of stitches){ if(s.x<minX)minX=s.x; if(s.x>maxX)maxX=s.x; if(s.y<minY)minY=s.y; if(s.y>maxY)maxY=s.y; }
