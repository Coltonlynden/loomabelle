...
const SAFE = () => document.querySelector('#safeMode')?.checked;
...
export async function quantizeSafe(imgData,k,mask){
  // In Safe Mode, avoid spawning a worker (lower peak memory / fewer crashes)
  if (SAFE()) {
    log('Safe Mode: quantizing on main thread','warn');
    return medianCutQuantize(imgData,k,mask);
  }

  if(!window._qWorkerURL){
    window._qWorkerURL=URL.createObjectURL(new Blob([`/* worker code unchanged */`],{type:'text/javascript'}));
  }
  try{
    ...
    worker.postMessage({img:imgData,k,mask});
    const out=await p; worker.terminate(); return out;
  }catch(err){
    log('Worker blocked/failure; using main thread','warn');
    return medianCutQuantize(imgData,k,mask);
  }
}
  try{
