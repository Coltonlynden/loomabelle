// tools + painting (no diagonal texture; overlay off by default)
(function(){
  const $  = (s,r=document)=>r.querySelector(s);
  const $$ = (s,r=document)=>Array.from(r.querySelectorAll(s));
  const S  = (window.EAS ||= {}).state ||= { tool:'brush', brushSize:18, mode:'mask', dirAngle:45, showDir:false };
  ...
  // undo/redo
  const UNDO=[], REDO=[];
  function pushUndo(){ try{ UNDO.push(mctx.getImageData(0,0,1024,1024)); if(UNDO.length>25) UNDO.shift(); REDO.length=0; }catch{} }
  $('#btn-undo')?.addEventListener('click',()=>{ if(UNDO.length){ REDO.push(mctx.getImageData(0,0,1024,1024)); mctx.putImageData(UNDO.pop(),0,0); window.EAS_preview.render(); }});
  $('#btn-redo')?.addEventListener('click',()=>{ if(REDO.length){ UNDO.push(mctx.getImageData(0,0,1024,1024)); mctx.putImageData(REDO.pop(),0,0); window.EAS_preview.render(); }});
  ...
  function dot(x,y,r,erase){
    ...
    mctx.beginPath(); mctx.arc(x,y,r,0,Math.PI*2); mctx.fillStyle='#000'; mctx.fill(); mctx.restore();
  }
  ...
  function wand(x,y){
    pushUndo();
    const W=1024,H=1024;
    const b=bctx.getImageData(0,0,W,H).data; const m=mctx.getImageData(0,0,W,H); const d=m.data;
    const idx=(X,Y)=>((Y|0)*W+(X|0))<<2; const i0=idx(x|0,y|0);
    const r0=b[i0], g0=b[i0+1], bl0=b[i0+2], tol=32;
    const Q=[[x|0,y|0]], seen=new Uint8Array(W*H);
    while(Q.length){
      const [X,Y]=Q.pop(); if(X<0||Y<0||X>=W||Y>=H) continue; const p=(Y*W+X); if(seen[p]) continue; seen[p]=1;
      const q=p<<2, r=b[q], g=b[q+1], bl=b[q+2]; if(Math.abs(r-r0)+Math.abs(g-g0)+Math.abs(bl-bl0)>tol) continue;
      d[q]=0; d[q+1]=0; d[q+2]=0; d[q+3]=255; Q.push([X+1,Y],[X-1,Y],[X,Y+1],[X,Y-1]);
    }
  ...
  function move(ev){
    if(!painting) return; const p=pos(ev,mask);
  ...
  $('#text-string')?.addEventListener('input',e=>{T.content=e.target.value;});
