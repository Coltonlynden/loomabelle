(function(){
  ...
  document.getElementById("btn-clear-mask").onclick=()=>{ mctx.clearRect(0,0,mask.width,mask.height); S.hasMask=false; EAS_processing.renderPreview(); };
  document.getElementById("btn-fill-mask").onclick =()=>{ mctx.fillStyle="rgba(0,0,0,1)"; mctx.fillRect(0,0,mask.width,mask.height); S.hasMask=true; EAS_processing.renderPreview(); };
  ...
  function down(ev){
    ...
    painting=true; const {x,y}=pos(ev);
    if(S.tool==="wand"){ wandFill(x|0,y|0); EAS_processing.renderPreview(); return; }
    drawPoint(x,y,S.tool==="erase"); EAS_processing.renderPreview();
  }
  function move(ev){
    if(!painting||S.tool==="wand") return;
    const {x,y}=pos(ev); drawPoint(x,y,S.tool==="erase"); EAS_processing.renderPreview();
  }
