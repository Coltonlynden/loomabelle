(function(){
  ...
  const baseC=document.getElementById("canvas");
  ...
  document.getElementById("btn-clear-mask").onclick=()=>{
    ...
    EAS_processing.computeEdges(); EAS_processing.renderPreview();
  ...
  document.getElementById("toggle-mask").onchange=e=>{ maskC.style.opacity=e.target.checked?"0.5":"0"; };
  ...
  function updateZoom(z){ S.zoom=Math.min(3,Math.max(0.4,z)); EAS_processing.setShellTransform(); zoomLabel.textContent=Math.round(S.zoom*100)+"%"; }
  zoomIn.onclick = ()=>updateZoom(S.zoom+0.1);
  zoomOut.onclick= ()=>updateZoom(S.zoom-0.1);
  zoomReset.onclick=()=>{ S.panX=0; S.panY=0; updateZoom(1); };
  ...
  function pos(ev){
    ...
    return {x:cx*(maskC.width/r.width), y:cy*(maskC.height/r.height)};
  }
  ...
  function down(ev){
    ...
    const {x,y}=pos(ev); drawCursor(x,y);
    ...
    if(S.brushMode==="mask"){
      ...
      painting=true; EAS_processing.paintDirection(x,y,S.brushSize,S.dirAngle); EAS_processing.renderPreview();
    }
  }
  function move(ev){
    const {x,y}=pos(ev); drawCursor(x,y);
    ...
    if(S.brushMode==="dir"){ EAS_processing.paintDirection(x,y,S.brushSize,S.dirAngle); EAS_processing.renderPreview(); }
  }
  function up(){
    if(painting){
      if(S.brushMode==="mask"){ EAS_processing.computeEdges(); EAS_processing.pushUndo(); }
    }
    painting=false;
  }
  ...
  // make text draggable on the Live preview
  const prev=document.getElementById("preview");
  prev.addEventListener("mousedown",e=>{
    if(S.brushMode!=="text") return;
