/* Loomabelle — compatibility build (no modules, ES5-ish) */
(function(){
  ...
  // Tabs + smooth scroll
  $$.call(null,'[data-scroll]').forEach(function(btn){
  ...
  var prev=document.createElement('canvas'); prev.width=480; prev.height=270; prevHost.innerHTML=''; prevHost.appendChild(prev); var pctx=prev.getContext('2d');
  var draw=document.createElement('canvas'); draw.width=480; draw.height=270; drawHost.innerHTML=''; drawHost.appendChild(draw); var dctx=draw.getContext('2d'); dctx.lineCap='round'; dctx.lineJoin='round';
  ...
  function processImage(img){
    ...
    var q=kmeans(x,8); // {labels}
    STATE.stitches = autoTrace(q.labels, w, h, prev.width, prev.height);
    render();
  }
  ...
  function kmeans(ctx,k){
    ...
    for(var it=0; it<6; it++){
      ...
      for(var c2=0;c2<k;c2++){ if(sums[c2][3]) centers[c2]=[sums[c2][0]/sums[c2][3], sums[c2][1]/sums[c2][3], sums[c2][2]/sums[c2][3]]; }
    }
  ...
  if(sw && sw.children.length===0){
    ['#fb7185','#f472b6','#d8b4fe','#a78bfa','#93c5fd','#38bdf8','#99f6e4','#5eead4','#fde68a','#facc15','#fca5a5','#86efac'].forEach(function(c){
      var d=document.createElement('div'); d.style.cssText='height:40px;width:40px;border-radius:999px;border:1px solid #fff;box-shadow:0 1px 2px rgba(0,0,0,.06);background:'+c+';cursor:pointer'; d.title=c; d.addEventListener('click',function(){STATE.active=c;}); sw.appendChild(d);
    });
  }
  ...
  function snapshot(){
    ...
    // trace drawing alpha to stitches
    var img=new Image();
    img.onload=function(){
  ...
  function encDST(){
    ...
    u.forEach(function(s){
      if(s.cmd==='stop'){ enc(0,0,0xC0); colors++; return; }
      if(s.cmd==='jump'){ enc(s.dx,s.dy,0x80); return; }
      enc(s.dx,s.dy,0);
    });
    bytes.push(0,0,0xF3);
  ...
  function encEXP(){
    ...
    u.forEach(function(s){
      if(s.cmd==='stop'){ put(0,0,'stop'); return; }
      if(s.cmd==='jump'){ put(s.dx,s.dy,'jump'); return; }
  ...
  // Hook up export buttons by visible text
  $$('.col.card.rose .formats .btn').forEach(function(btn){
    ...
    btn.addEventListener('click', function(){
      try{
        ...
        encViaAI(fmt, function(err, bytes){
          if(err) return alert(fmt+': '+err.message);
  ...
  render();
  console.log('Loomabelle compatibility script initialized.');
})();