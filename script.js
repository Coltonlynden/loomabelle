...
const on=(e,ev,fn,o)=> e && e.addEventListener(ev,fn,o||{passive:false}); const DPR=()=>window.devicePixelRatio||1; const clamp=(v,mi,ma)=>Math.max(mi,Math.min(ma,v));
...
function init(){
  ...
  function activate(t){ tabBtns.forEach(b=>b.classList.toggle('active',b.dataset.tab===t)); panels.forEach(p=>p.classList.toggle('active',p.dataset.panel===t)); if(t==='upload') queueResize(); }
  ...
  const upPanel=$('.panel[data-panel="upload"]'), drawPanel=$('.panel[data-panel="draw"]');
  const upZone=upPanel?.querySelector('.upload-zone'); const fileInput=upPanel?.querySelector('input[type=file]');
  const prevCard=upPanel?.querySelector('.card.rose'); const prevArea=upPanel?.querySelector('.preview'); const formats=upPanel?.querySelector('.formats');
  const drawHost=drawPanel?.querySelector('.canvas'); const drawToolbar=drawPanel?.querySelector('.toolbar'); const swatchesBox=drawPanel?.querySelector('.swatches');

  if(fileInput){ fileInput.removeAttribute('disabled'); fileInput.accept='image/*,.heic,.heif,.png,.jpg,.jpeg,.gif'; }

  const prevCanvas=ensureCanvas(prevArea); const pctx=prevCanvas.getContext('2d',{willReadFrequently:true});
  const drawCanvas=ensureCanvas(drawHost); const dctx=drawCanvas.getContext('2d',{willReadFrequently:true}); dctx.lineCap='round'; dctx.lineJoin='round';

  function fit(cnv, host){ let w=Math.max(320, host.clientWidth||host.parentElement?.clientWidth||640); let h=Math.max(220, Math.round(w*9/16)); const s=DPR(); cnv.style.width=w+'px'; cnv.style.height=h+'px'; cnv.width=Math.round(w*s); cnv.height=Math.round(h*s); cnv.getContext('2d').setTransform(s,0,0,s,0,0); }
  function queueResize(){ setTimeout(()=>{ fit(prevCanvas,prevArea); fit(drawCanvas,drawHost); redraw(); },0); }
  const ro=new ResizeObserver(()=>{ fit(prevCanvas,prevArea); fit(drawCanvas,drawHost); redraw(); }); prevArea&&ro.observe(prevArea); drawHost&&ro.observe(drawHost); queueResize();
  ...
  const progBar=document.createElement('div'); progBar.style.cssText='height:100%;width:0%;background:#111827;opacity:.9'; progWrap.appendChild(progBar); ensurePosition(prevCard).appendChild(progWrap);
  ...
  const tools=document.createElement('div'); tools.style.cssText='position:absolute;left:12px;bottom:12px;display:flex;gap:10px;flex-wrap:wrap;z-index:3;visibility:hidden;';
  const btnProcess=makeBtn('Process Photo'); const btnHighlight=makeBtn('Highlight Subject'); const lblNo=document.createElement('label'); const chkNo=document.createElement('input'); chkNo.type='checkbox'; lblNo.appendChild(chkNo); lblNo.appendChild(document.createTextNode(' No subject'));
  tools.append(btnProcess,btnHighlight,lblNo); ensurePosition(prevCard).appendChild(tools);
  ...
  if(drawToolbar){ Array.from(drawToolbar.children).forEach((b,i)=>{ b.removeAttribute('disabled'); if(i===0) b.classList.add('active'); }); const [btnPen,btnEraser]=[drawToolbar.children[0],drawToolbar.children[1]];
    ...
    const btnPD=document.createElement('button'); btnPD.className='btn soft'; btnPD.textContent='Process Drawing'; drawToolbar.appendChild(btnPD); on(btnPD,'click', processDrawingFlow);
  }
  ...
  function ensureCanvas(host){ const c=document.createElement('canvas'); c.style.display='block'; if(getComputedStyle(host).position==='static') host.style.position='relative'; if(host.clientHeight<180) host.style.minHeight='220px'; host.appendChild(c); return c; }
  function ensurePosition(host){ const cs=getComputedStyle(host); if(cs.position==='static') host.style.position='relative'; return host; }

  async function loadImageFile(file){
    if(!/\.(jpg|jpeg|png|gif|heic|heif)$/i.test(file.name||'')){ alert('Please choose a JPG, PNG, GIF, HEIC/HEIF image.'); return; }
    ...
    try{
      ...
      activate('upload'); fit(prevCanvas,prevArea); renderBaseImage(); STATE.stitches.length=0; renderStitches(); tools.style.visibility='visible'; setFormatsVisible(false);
  ...
  function renderBaseImage(){
    const img=STATE.image; if(!img) return; fit(prevCanvas,prevArea);
    const W=img.width,H=img.height; const Wp=prevCanvas.width/DPR(), Hp=prevCanvas.height/DPR(); const s=Math.min(Wp/W, Hp/H), w=W*s, h=H*s, ox=(Wp-w)/2, oy=(Hp-h)/2;
    const tmp=document.createElement('canvas'); tmp.width=W; tmp.height=H; tmp.getContext('2d').putImageData(img,0,0);
    const ctx=prevCanvas.getContext('2d'); ctx.setTransform(DPR(),0,0,DPR(),0,0); ctx.clearRect(0,0,Wp,Hp); ctx.fillStyle='#fff'; ctx.fillRect(0,0,Wp,Hp); ctx.drawImage(tmp,ox,oy,w,h);
  ...
  function renderStitches(){
    ...
    let started=false; for(const s of STATE.stitches){ if(s.move){ ctx.moveTo(s.x,s.y); started=true; } else if(started){ ctx.lineTo(s.x,s.y); } } ctx.stroke(); ctx.restore();
  }
  ...
  function kmeansPalette(imgData,k,mask){
    const W=imgData.width,H=imgData.height,d=imgData.data; const step=Math.max(1,Math.floor(Math.sqrt((W*H)/20000))); const pts=[];
  ...
  async function quantizeFast(imgData,k,mask){
    const W=imgData.width,H=imgData.height,d=imgData.data; const palette=kmeansPalette(imgData,Math.min(k,6),mask); const indexed=new Uint8Array(W*H);
  ...
  function planStitches(data,opts){
    ...
    const rad=angle*Math.PI/180, sin=Math.sin(rad), cos=Math.cos(rad); const bands=Math.max(4,Math.floor(Math.min(W,H)/24));
    for(let b=0;b<bands;b++){
      const t=(b/bands)*(W+H);
      for(let y=0;y<H;y+=step){
        const x0=Math.floor(t - y*(sin/cos)); let inRun=false,lx=null,ly=null;
        for(let px=x0-20; px<=x0+20; px++){
          const xx=px, yy=Math.floor(y + (px-x0)*(sin/cos));
          if(xx>=0&&xx<W&&yy>=0&&yy<H){ if(!inRun){ ops.push({cmd:'jump',x:xx,y:yy}); inRun=true; } lx=xx; ly=yy; }
        }
  ...
  function toUnits(ops,pxPerMm,outW,outH){ const s=1/pxPerMm*10, cx=outW/2, cy=outH/2; const out=[]; let prev=null;
    for(const op of ops){
  ...
  function writeDST(ops,opts){
    const pxPerMm=opts?.pxPerMm||2, outW=opts?.outW||640, outH=opts?.outH||360; const u=toUnits(ops,pxPerMm,outW,outH), bytes=[];
    ...
    let colors=0;
    for(const s of u){ if(s.cmd==='stop'){ enc(0,0,0xC0); colors++; continue; } if(s.cmd==='jump'){ enc(s.dx,s.dy,0x80); continue; } enc(s.dx,s.dy,0); }
    bytes.push(0,0,0xF3);
  ...
  function hookDownloads(res){
    fmtBtns.forEach(btn=>{ const fmt=(btn.textContent||'').trim().toUpperCase();
  ...
  async function processDrawingFlow(){
    setFormatsVisible(false); setProgress(1);
    const w=drawCanvas.width,h=drawCanvas.height,sctx=drawCanvas.getContext('2d'); const id=sctx.getImageData(0,0,w,h);
  ...
  function paintSwatches(pal){
    ...
    pal.forEach((c,i)=>{
      const sw=document.createElement('button'); sw.className='swatch'; const [r,g,b]=c;
  ...
  function setProgress(p){ /* set by closure in scope above */ }
