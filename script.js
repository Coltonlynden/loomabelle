/* Loomabelle — v9.5
   - Prevents any refresh/submit (buttons defaulted to type="button", forms blocked)
   - Photo thumbnail renders inside the "Upload" card (replaces dropzone)
   - "Preview (stitched)" shows stitches only (no original photo)
   - Progress spinner + bar drawn inside the Preview canvas while processing
   - Stable canvas sizing across tab switches and iOS (no infinite growth)
*/
(function(){
  // ---------- tiny helpers ----------
  ...
  document.addEventListener('keydown', function(e){
    if(e.key==='Enter' && e.target && /INPUT|SELECT/.test(e.target.tagName)) { e.preventDefault(); }
  }, true);

  // ---------- minimal CSS (no visual theme change) ----------
  (function addCSS(){
    var css=""
      ...
      +".lb-toolbar{position:absolute;left:8px;right:8px;bottom:18px;display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;z-index:2000}"
      +".lb-progress{position:absolute;left:0;right:0;bottom:0;top:0;display:none;align-items:center;justify-content:center;z-index:2001}"
      +".lb-progress .ring{width:52px;height:52px;border-radius:9999px;border:4px solid rgba(0,0,0,.1);border-top-color:#9ac2ff;animation:spin 1s linear infinite}"
  ...
  var STATE={
    ...
    history:[], ai:{}, stitches:[],
    lastImage:null, imgFit:null, // fit inside UPLOAD thumb
    ...
    lastPreviewSize:{w:640,h:360}, lastDrawSize:{w:640,h:360},
    _sizingPass:0,
  };

  // ---------- config from query/localStorage ----------
  (function cfg(){
  ...
  // ---------- tab wiring (keep original appearance) ----------
  function wireTabs(){
    ...
    tabs.forEach(function(btn){
      ...
      on(btn,'click',function(e){
        ...
        setTimeout(resizeAll, 30); setTimeout(resizeAll, 120);
      });
    });
    // Quick-links
    $$('a,button').forEach(function(el){
      ...
      if(t.indexOf('start with a photo')>-1 || t.indexOf('upload photo')>-1){
        on(el,'click',function(e){ e.preventDefault(); ($('.tab-btn[data-tab="upload"]')||tabs[0]).click(); });
      }
      if(/open.*draw|open.*drawing|draw & trace/.test(t)){
        on(el,'click',function(e){ e.preventDefault(); ($('.tab-btn[data-tab="draw"]')||tabs[1]).click(); });
      }
    });
  ...
  // ---------- host elements ----------
  var uploadCard=$('.panel[data-panel="upload"]')||document,          // container
      uploadZone=uploadCard.querySelector('.upload-zone')||$('.upload-zone',uploadCard),
      previewHost=$('.col.card.rose .preview') || $('.preview'),
      drawHost=$('.panel[data-panel="draw"] .canvas') || $('.canvas');

  if(!uploadZone || !previewHost || !drawHost){ console.error('Missing required hosts'); return; }
  if(getComputedStyle(previewHost).position==='static'){ previewHost.style.position='relative'; }
  if(getComputedStyle(uploadZone).position==='static'){ uploadZone.style.position='relative'; }

  // --- PREVIEW canvas (stitches only) ---
  ...
  // progress overlay INSIDE preview
  var prog=make('div','lb-progress');
  var ring=make('div','ring');
  var barwrap=make('div','barwrap'); var bar=make('div','bar'); barwrap.appendChild(bar);
  prog.appendChild(ring); prog.appendChild(barwrap);
  previewHost.appendChild(prog);

  // --- UPLOAD thumbnail canvas (shows the selected photo) ---
  ...
  // Replace the drop area’s inner content but keep its border/background
  var dropInner = document.createElement('div'); dropInner.style.pointerEvents='none';
  uploadZone.innerHTML=''; uploadZone.appendChild(photoCanvas);

  // --- DRAW canvas (user sketch) ---
  var draw=document.createElement('canvas'), dctx=draw.getContext('2d',{alpha:true,willReadFrequently:true});
  draw.classList.add('loomabelle-interactive'); dctx.lineCap='round'; dctx.lineJoin='round';
  ...
  // ---------- sizing (inner content width to avoid feedback growth) ----------
  ...
  var resizeAll=debounce(function(){
    ...
    updatePxPerMm(); render();
    STATE._sizingPass=0;
  },50);
  try{ new ResizeObserver(resizeAll).observe(previewHost); new ResizeObserver(resizeAll).observe(drawHost); new ResizeObserver(resizeAll).observe(uploadZone); }catch(e){}
  on(window,'resize',resizeAll);
  resizeAll();

  // ---------- palette clicks (left column) ----------
  var sw=$('.swatches'); if(sw && !sw.children.length){
    sw.style.display='flex'; sw.style.flexWrap='wrap'; sw.style.gap='12px';
  ...
  on(fileInput,'change',function(e){ var f=fileInput.files && fileInput.files[0]; if(f) loadImage(f); });
  ...
  // Upload checkboxes -> options
  (function wireUploadCheckboxes(){
    var boxes = $$('.panel[data-panel="upload"] input[type="checkbox"]') || [];
  ...
  on(btnProcess,'click',function(e){
    ...
    if(!STATE.lastImage){ if(fileInput.files && fileInput.files[0]) return loadImage(fileInput.files[0]); alert('Choose a photo first.'); return; }
    processPhoto(STATE.lastImage);
  });

  // ---------- photo load -> show inside UPLOAD ----------
  function showPhotoInUpload(img){
    sizeCanvas(photoCanvas,uploadZone,'lastUploadSize');
    var W=photoCanvas.width/dpr(), H=photoCanvas.height/dpr();
  ...
  function loadImage(file){
    var img=new Image();
    img.onload=function(){ STATE.lastImage=img; STATE.processed=false; setFormatsVisible(false); showPhotoInUpload(img); if(STATE.opts.autoTrace){ STATE.subject.enabled=true; } render(); };
    img.onerror=function(){ alert('Could not load image'); };
    img.crossOrigin='anonymous'; img.src=URL.createObjectURL(file);
  }

  // ---------- scroll lock for selection/drawing ----------
  ...
  function canvasPoint(e, canvas){
    ...
    var scaleX=(canvas.width/dpr())/rect.width, scaleY=(canvas.height/dpr())/rect.height;
    return {x:cssX*scaleX, y:cssY*scaleY};
  }

  // ---------- highlight subject on UPLOAD photo ----------
  ...
  on(photoCanvas,'pointerdown',function(e){
    ...
    var pt=canvasPoint(e,photoCanvas); start=[pt.x,pt.y]; highlighting=true;
    STATE.subject.rect={x:start[0],y:start[1],w:0,h:0}; drawSelection(); // immediate feedback
  },{passive:false});
  on(photoCanvas,'pointermove',function(e){
    if(!highlighting || !STATE.subject.enabled) return;
    e.preventDefault();
    var pt=canvasPoint(e,photoCanvas);
  ...
  on(photoCanvas,'touchmove',preventTouch,{passive:false});

  function drawSelection(){
    if(!STATE.lastImage){ photoCtx.clearRect(0,0,photoCanvas.width,photoCanvas.height); return; }
    showPhotoInUpload(STATE.lastImage);
    if(STATE.subject.enabled && STATE.subject.rect){
      var r=STATE.subject.rect, W=photoCanvas.width/dpr(), H=photoCanvas.height/dpr();
      photoCtx.save();
  ...
  function render(){
    ...
    for(var i=0;i<STATE.stitches.length;i++){ var s=STATE.stitches[i]; if(s.cmd==='jump') ctx.moveTo(s.x,s.y); else ctx.lineTo(s.x,s.y); } ctx.stroke();
  }
  ...
  function processPhoto(img){
    ...
    try{
      ...
      if((STATE.opts.reduce||STATE.opts.cleanup)){
        try{
          ...
          if(STATE.opts.reduce){
            ...
            var finer=marchingSquares(fg,ref.w,ref.h); if(finer && finer.length) paths=finer;
          }
        }catch(e){}
      }
      ...
      console.error(err);
      STATE.busy=false; STATE.processed=false; STATE.stitches=[]; setFormatsVisible(false); progressDone(); render();
  ...
  function processDrawing(){
    ...
    try{
      ...
      ctx.putImageData(g,0,0); progressSet(45);
      ...
      var paths=marchingSquares(mask,sw,sh); progressSet(85);
  ...
  // ---------- Draw tool processing button already wired. ----------
  // ---------- Init ----------
  function init(){ wireTabs(); render(); }
  init();
})();