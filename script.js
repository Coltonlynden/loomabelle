...
(function(){
  ...
  // ===== show image in preview BEFORE processing =====
  ...
  function wireDrawTools(){
    ...
    var proc=make('button','btn','Process Drawing'); proc.style.marginLeft='10px'; tb&&tb.appendChild(proc);
  ...
  function cropMaskToSubject(mask,w,h){ var f=STATE.imgFit, r=STATE.subject.rect; if(!f||!r) return; var sx=Math.max(0,Math.floor((r.x-f.ox)/f.scale)), sy=Math.max(0,Math.floor((r.y-f.oy)/f.scale)), ex=Math.min(w-1,Math.floor((r.x+r.w-f.ox)/f.scale)), ey=Math.min(h-1,Math.floor((r.y+r.h-f.oy)/f.scale)); for(var y=0;y<h;y++){ for(var x=0;x<w;x++){ if(!(x>=sx&&x<=ex&&y>=sy&&y<=ey)) mask[y*w+x]=0; } } }
