/* Loomabelle — v8 (ES5, single-file, offline)
   Highlights:
   • Preview shows uploaded photo immediately
   • Thread palette restored (and preserved if you already had one)
   • Draw → Process switches to Preview tab to display result
   • Real progress bar in Preview; non-blocking processing via Web Worker
   • Robust, fast pipeline with safe downscale; subject highlight optional
   • Exports: DST/EXP offline; optional AI for PES/JEF (unchanged)
*/
(function(){
  // ---------- tiny helpers ----------
  ...
  // Hide any “mockup” helper notes (no HTML/CSS edits)
  ...
  (function wireTabs(){
    ...
    // “Start with a photo” / “Upload photo”
  ...
  // progress bar overlay (inside Preview)
  ...
  var tb=make('div'); tb.style.cssText='position:absolute;left:12px;bottom:12px;display:flex;gap:10px;flex-wrap:wrap;z-index:3';
  ...
  function loadImage(file){
    ...
    img.onload=function(){
      ...
      showImageInPreview(img,1); // critical: show immediately
      switchTo('upload'); // keep UX consistent
    };
  ...
  (function wireDraw(){
    ...
    on(proc,'click',function(){ processDrawing(); switchTo('upload'); }); // show result in preview
  })();
  on(draw,'pointerdown',function(e){
    ...
    dctx.strokeStyle=STATE.active; dctx.lineWidth=3; dctx.beginPath(); dctx.moveTo(x,y); dragging=true;
  });
  ...
  function undo(){
    // lightweight clear last stroke (best-effort)
  ...
  function render(){
    ...
    // Keep current image already drawn by showImageInPreview()
    // Draw grid/guides on top if toggled
    if(STATE.guides){
  ...
  // ---------- Worker for processing (no UI freeze) ----------
  var workerURL=(function(){
    var code = [
      ...
      '};',
      // utils in worker:
      'function otsu(gray){ var w=gray.w,h=gray.h,g=gray.data, hist=new Uint32Array(256); for(var i=0;i<w*h;i++) hist[g[i]]++; var sum=0,total=w*h; for(i=0;i<256;i++) sum+=i*hist[i]; var sumB=0,wB=0,maxV=0,thr=127; for(i=0;i<256;i++){ wB+=hist[i]; if(!wB) continue; var wF=total-wB; if(!wF) break; sumB+=i*hist[i]; var mB=sumB/wB, mF=(sum-sumB)/wF; var v=wB*wF*(mB-mF)*(mB-mF); if(v>maxV){maxV=v;thr=i;} } return thr; }',
      'function toGray(img){ var w=img.w,h=img.h,d=img.data, g=new Uint8Array(w*h); for(var i=0;i<w*h;i++){ var p=i*4; g[i]=(0.299*d[p]+0.587*d[p+1]+0.114*d[p+2])|0; } return {w:w,h:h,data:g}; }',
      ...
      'function processPhoto(d){ var img=d.image; var w=img.w,h=img.h; var step=d.step||function(){}; step(10); var gray=toGray(img); var thr=otsu(gray); var mask=new Uint8Array(w*h); for(var i=0;i<w*h;i++) mask[i]=(gray.data[i]>thr)?0:1; var m={w:w,h:h,data:mask}; if(d.subject && !d.subject.no){ cropMask(m,d.subject); } step(40); var p1=marchingSquares(m); if((d.reduce||d.cleanup)){ var img2={w:w,h:h,data:new Uint8ClampedArray(img.data)}; if(d.cleanup){ blur1(img2); } var labels=kmeansLabels(img2,6); var m2=darkestMask(img2,labels,6); if(d.subject && !d.subject.no){ cropMask(m2,d.subject); } var p2=marchingSquares(m2); if(p2 && p2.length) p1=p2; } step(90); return p1; }',
      'function processDrawing(d){ var w=d.w,h=d.h, alpha=d.alpha; var mask=new Uint8Array(w*h); for(var i=0;i<w*h;i++) mask[i]= alpha[i]>0?1:0; return marchingSquares({w:w,h:h,data:mask}); }'
    ].join('\n');
  ...
  on(btnProcess,'click',function(){
    if(!STATE.lastImage){ if(fileInput && fileInput.files && fileInput.files[0]) return loadImage(fileInput.files[0]); alert('Choose a photo first.'); return; }
    processPhoto();
  });

  function processPhoto(){
    if(STATE.busy) return; STATE.busy=true; STATE.processed=false; setFormatsVisible(false); setProgress(1);
    // Build image bitmap for worker at safe size
    var max=1024, img=STATE.lastImage;
    // Resize into an offscreen canvas to extract RGBA
    ...
    worker.postMessage({
      kind:'photo',
      image:{w:w,h:h,data:id.data},
      reduce:STATE.opts.reduce,
      cleanup:STATE.opts.cleanup,
      subject:subj
    });
  }
  ...
  function processDrawing(){
    if(STATE.busy) return; STATE.busy=true; STATE.processed=false; setFormatsVisible(false); setProgress(1);
    var w=draw.width, h=draw.height, ctx=dctx.getImageData(0,0,w,h).data, alpha=new Uint8Array(w*h);
    for(var i=0;i<w*h;i++){ alpha[i]=ctx[i*4+3]; }
    // Downscale if large
    var max=1024, s=Math.min(1, max/Math.max(w,h)), sw=(w*s)|0, sh=(h*s)|0;
    var tmp=document.createElement('canvas'); tmp.width=sw; tmp.height=sh; var t=tmp.getContext('2d');
    t.drawImage(draw,0,0,sw,sh);
    var id2=t.getImageData(0,0,sw,sh).data, a2=new Uint8Array(sw*sh);
    for(i=0;i<sw*sh;i++){ a2[i]=id2[i*4+3]; }
    worker.postMessage({ kind:'drawing', w:sw, h:sh, alpha:a2 });
  }
  ...
  // ---------- init ----------
  (function init(){
    // prepare preview background to avoid blank look
