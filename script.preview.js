import { getCurrentLayers, fitCanvasesPublic } from './script.draw.js';
...
window.addEventListener('resize', ()=>{ fit(); fitCanvasesPublic(); });
fit();
...
genBtn.addEventListener('click', ()=>{
  ...
  cx.globalCompositeOperation='destination-in'; cx.drawImage(maskCanvas,0,0);
  cx.globalCompositeOperation='source-over'; cx.drawImage(textCanvas,0,0);
...
function drawPreview(data){
  ...
  let rx=(W-pad*2)/2, ry=(H-pad*2)/2; if(arHoop>arCanvas){ ry=rx/arHoop; } else { rx=ry*arHoop; }
...
/* ---- Raster → simple running stitches (direction aware) ---- */
function rasterToStitches(bmp, dir){
  ...
  const theta = (dir.angle*Math.PI)/180, dx=Math.cos(theta), dy=Math.sin(theta);
...
/* Minimal DST (experimental): writes relative stitches as CSV-flavored .dst for many viewers. */
